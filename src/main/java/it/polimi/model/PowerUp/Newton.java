...
public class Newton extends PowerUpCard {
    ...
    public void effect2(GameModel gameModel, int rd, int cd, Player target){
        Square destinationSquare = gameModel.getMap().searchSquare(rd,cd);
        Square actualSquare = gameModel.getMap().searchSquare(target.getRow(),target.getColumn());
        if((gameModel.getMap().sameDirection(destinationSquare,actualSquare)) && (3>gameModel.getMap().distance(destinationSquare,actualSquare))){
            gameModel.getMap().movePlayer(target,destinationSquare);
        }
    }
}
